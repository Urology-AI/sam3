#!/usr/bin/env python3
"""
track_nerves_multi.py
=====================

Like track_nerves.py but accepts multiple annotation JSONs for the same video.
Each JSON is processed with the same SAM2 predictor (loaded once) and gets its
own output subdirectory named after the JSON file stem.

Usage:

python3 track_nerves_multi.py \
  --video aua_videos/case.mp4 \
  --tracks aua_boxes/case_instrument.json aua_boxes/case_prostate.json \
  --sbs_eye none \
  --render_from_first_track

Output layout:
  inferred_videos/<video_stem>_mask_tracks/
    case_instrument/
      masks/track_00/ ...
      overlay.mp4
    case_prostate/
...
def draw_label_panel(frame: np.ndarray, entries, panel_x: int, panel_y: int):
    ...
    for i, (label, color_bgr) in enumerate(entries):
        ...
        cv2.putText(
            frame, label, (tx + 1, ty + 1), font, font_scale,
            (0, 0, 0), thickness + 1, cv2.LINE_AA,
        )
        cv2.putText(
            frame, label, (tx, ty), font, font_scale,
...
def parse_args():
    ...
    ap.add_argument(
        ...
        required=True,
        nargs="+",
        help="One or more JSON files exported from annotate_mask_tracks.html. "
             "Each gets its own output subdirectory named after the JSON stem.",
    )

    ap.add_argument(
        "--output_dir",
        default=None,
        help="Base output directory. Each JSON gets a subdirectory inside. "
             "If omitted, creates inferred_videos/<video>_mask_tracks/",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.30,
        help="Overlay opacity in output video. Default: 0.30",
    )
...
def main():
    ...
    # ── Video metadata (shared across all annotation files) ────────────────────
    ...
    print(f"Frames : {n_frames}  FPS : {fps:.3f}  Size : {video_w}x{video_h}")
    print(f"Tracks files : {len(args.tracks)}")
    ...
    # ── GPU / SAM2 (loaded once, reused for all annotation files) ──────────────
    ...
    # ── Pre-scan: intersection render window across all JSONs ──────────────────

    ann_windows = []  # (path, first_frame, last_frame)
    for tracks_path in args.tracks:
        with open(tracks_path) as f:
            ann_pre = json.load(f)
        pre_tracks = [t for t in ann_pre.get("tracks", []) if t.get("masks")]
        if not pre_tracks:
            continue
        first = min(
            int(t.get("start_frame") or min(int(m["frame"]) for m in t["masks"]))
            for t in pre_tracks
        )
        last = max(
            int(t.get("end_frame") or max(int(m["frame"]) for m in t["masks"]))
            for t in pre_tracks
        )
        ann_windows.append((tracks_path, first, last))

    if not ann_windows:
        print("No usable tracks found in any annotation file.")
        del predictor
        return

    first_frames = [f for _, f, _ in ann_windows]
    last_frames  = [l for _, _, l in ann_windows]

    if len(set(first_frames)) > 1:
        print("\nWARNING: annotation files start at different frames:")
        for path, f, _ in ann_windows:
            print(f"  {os.path.basename(path)}: frame {f}  ({f / fps:.1f}s)")

    intersection_start = max(first_frames)
    intersection_end   = min(last_frames)

    if intersection_end < intersection_start:
        raise RuntimeError(
            f"No overlapping frames across annotation files "
            f"(intersection_start={intersection_start} > intersection_end={intersection_end}). "
            f"Check that your annotations cover the same region of the video."
        )

    print(
        f"\nIntersection render window: frames {intersection_start}–{intersection_end} "
        f"({intersection_start / fps:.1f}s – {intersection_end / fps:.1f}s)"
    )

    # ── Per-annotation-file loop ───────────────────────────────────────────────

    for tracks_path in args.tracks:
        tracks_stem = os.path.splitext(os.path.basename(tracks_path))[0]
        output_dir = os.path.join(base_out, tracks_stem)
        masks_root = os.path.join(output_dir, "masks")

        os.makedirs(masks_root, exist_ok=True)

        print(f"\n{'═' * 70}")
        print(f"Annotation : {tracks_path}")
        ...
        if not tracks:
            print("No tracks in JSON — skipping.")
            continue
        ...
        if not prepared_tracks:
            print("No usable tracks after filtering — skipping.")
            continue
        ...
        print(f"\nInference complete for {tracks_stem}: {total_mask_count} total masks in {elapsed:.1f}s")
        ...
        # ── Rendering range: always the intersection across all JSONs ─────────────

        render_start = max(0, intersection_start)
        render_end   = min(n_frames, intersection_end + 1)

        print(
            f"Rendering intersection window: frames {render_start}–{render_end - 1} "
            f"({render_start / fps:.1f}s – {(render_end - 1) / fps:.1f}s)"
        )
