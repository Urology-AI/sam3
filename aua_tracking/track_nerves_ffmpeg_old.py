#!/usr/bin/env python3
"""
track_nerves_ffmpeg.py
======================

Same as track_nerves.py but uses ffmpeg for all video frame reading
instead of OpenCV seeking. Fixes a 2x timestamp mismatch in some MOV
files (e.g. Mochita_3D.mov) where OpenCV's frame-count seeking lands
on the wrong frame while ffmpeg's presentation-time seeking is correct.

The annotation JSON fps field converts frame indices to timestamps
(frame / fps_ann = time_s) which are passed to ffmpeg for accurate
seeking.
...
class LazyMaskCache:
    ...
    Keeps at most two keyframes in RAM at any time — the surrounding pair
    used for linear interpolation — so memory is O(1) regardless of video length.
    """
    ...
    def _evict_before(self, fidx: int) -> None:
        stale = [k for k in self._cache if k < fidx]
        for k in stale:
            del self._cache[k]
...
def main():
    ...
    for track in prepared_tracks:
        ...
        print(
            ...
            f"{len(mask_cache.kf_idxs)} rendered mask frames"
        )
    ...
    for fidx in range(render_start, render_end):
        ...
        for track in prepared_tracks:
            ...
            mask_f = td["mask_cache"].get_mask_at(fidx)
