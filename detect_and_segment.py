...
=====================
Zero-prompt prostate video segmentation using the fine-tuned SAM3 detector.

Pipeline
--------
  1. Run the fine-tuned SAM3 detector on a single frame (default: frame 0)
     to produce a bounding box — no manual annotation needed.
  2. Feed that box to the SAM3 tracker (same pipeline as sam3_bbox_segment.py)
     to propagate the mask through the entire video.

Usage
-----
  python3 detect_and_segment.py --video /path/to/video.mp4 \
      --detector_ckpt detector_training/checkpoints/<ts>/checkpoint_best.pth

  python3 detect_and_segment.py --video /path/to/video.mp4 \
      --detector_ckpt detector_training/checkpoints/<ts>/checkpoint_best.pth \
      --detect_frame 5 --output_dir /path/to/output
"""
...
DETECTOR_IMG_SIZE = 1008   # SAM3 ViT input size
...
# ── Memory helpers (identical to sam3_bbox_segment.py) ────────────────────────
...
# ── Streaming video loader (identical to sam3_bbox_segment.py) ────────────────
...
def clip_video(src: str, start_s: float, end_s: float, out_path: str) -> str:
    """
    Extract [start_s, end_s) from src using ffmpeg and write to out_path.
    Fast-seek (-ss before -i) + re-encode for frame accuracy.
    Returns out_path.
    """
...
def run_detector(detector_ckpt: str, video_path: str, frame_idx: int,
                 device: torch.device):
    """
    Run the fine-tuned SAM3 detector on one frame.

    Returns
    -------
    box_norm_xyxy : np.ndarray (4,) — [x1,y1,x2,y2] normalised to [0,1]
                    (what the tracker expects with rel_coordinates=True)
    box_abs_xyxy  : np.ndarray (4,) — same box in absolute video pixels
    conf          : float — sigmoid confidence of the selected query
    """
    ...
    # Read video metadata + the detection frame
    cap   = cv2.VideoCapture(video_path)
    ...
    img_t = img_t.unsqueeze(0).to(device)   # (1, 3, 1008, 1008)
...
def main():
    parser = argparse.ArgumentParser(
        description="Zero-prompt prostate video segmentation via SAM3 detector + tracker"
    )
    ...
    parser.add_argument("--output_dir",     default=None,
                        help="Output directory (default: <video_stem>_sam3_detected/)")
    parser.add_argument("--clip_start",     type=float, default=None,
                        help="Clip start time in seconds (requires --clip_end)")
    parser.add_argument("--clip_end",       type=float, default=None,
                        help="Clip end time in seconds (requires --clip_start)")
    parser.add_argument("--detect_frame",   type=int, default=0,
                        help="Frame index (within the clip/video) to run detector on (default: 0)")
    ...
    parser.add_argument("--mask_color",     default="0,255,0",
                        help="Mask overlay colour as R,G,B integers (default: 0,255,0 = green)")
    ...
    # Parse mask colour: "R,G,B" → BGR tuple for cv2
    try:
    ...
    mask_color_bgr = (b, g, r)   # cv2 uses BGR

    if args.output_dir is None:
        stem = os.path.splitext(os.path.basename(args.video))[0]
        args.output_dir = os.path.join(
            os.path.dirname(args.video), stem + "_sam3_detected"
        )
    ...
    print(f"Detect frame:   {args.detect_frame}")
    print(f"Mask color RGB: {r},{g},{b}  alpha: {args.mask_alpha}")
    print(f"Output:         {args.output_dir}")

    # ── [0] Clip (optional) ───────────────────────────────────────────────
    video_path = args.video   # may be replaced by the clip below
    ...
    # ── [2/6] Detector ────────────────────────────────────────────────────
    print(f"\n[2/6] Running detector on frame {args.detect_frame}...")
    t0 = time.time()
    box_norm_xyxy, box_abs_xyxy, conf = run_detector(
        args.detector_ckpt, video_path, args.detect_frame, device
    )
    print(f"  Detection done in {time.time()-t0:.1f}s")
    print(f"  Confidence:      {conf:.3f}" + ("  [LOW]" if conf < args.conf_threshold else ""))
    print(f"  Box (norm xyxy): {box_norm_xyxy.tolist()}")
    print(f"  Box (abs xyxy):  {box_abs_xyxy.tolist()}")
    if conf < args.conf_threshold:
        print(f"  [WARNING] conf={conf:.3f} < threshold={args.conf_threshold}. "
              f"Detection may be unreliable — consider a different --detect_frame.")
    gpu_check("after detector")
    cpu_check("after detector")

    # ── [3/6] Tracker ─────────────────────────────────────────────────────
    print(f"\n[3/6] Building SAM3 tracker...")
    t0 = time.time()
    ...
    # ── [4/6] Inference state ─────────────────────────────────────────────
    print(f"\n[4/6] Initializing inference state (streaming)...")
    t0 = time.time()
    ...
    # ── [5/6] Box prompt + propagate ─────────────────────────────────────
    print(f"\n[5/6] Adding detected box as prompt on frame {args.detect_frame}...")
    t0 = time.time()
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        _, _, _low_res, video_res_masks = predictor.add_new_points_or_box(
            inference_state=inference_state,
            frame_idx=args.detect_frame,
            obj_id=1,
            box=box_norm_xyxy,   # [0,1] xyxy — rel_coordinates=True by default
        )
    mask0 = (video_res_masks[0] > 0.0).cpu().numpy().squeeze()
    print(f"  Prompt mask: {mask0.sum()} px, coverage {mask0.mean()*100:.1f}%  "
          f"({time.time()-t0:.1f}s)")

    print(f"\n[6/6] Propagating through {n_frames} frames...")
    t0 = time.time()
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        for out_frame_idx, _obj_ids, _low_res, out_video_masks, _scores in propagate_with_guard(
            predictor, inference_state, start_frame_idx=args.detect_frame
        ):
    ...
    print(f"  Propagated in {time.time()-t0:.1f}s — {mask_count} masks saved")
    ...
    for fidx in range(frame_count):
        ...
        if fidx == args.detect_frame:
            x1, y1, x2, y2 = box_abs_xyxy.astype(int)
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 2)
            cv2.putText(frame, f"det conf={conf:.2f}", (x1, max(y1 - 10, 10)),
    ...
    print(f"  Detector ckpt: {args.detector_ckpt}")
    print(f"  Detect frame:  {args.detect_frame}  (conf={conf:.3f})")
    print(f"  Box (abs):     {box_abs_xyxy.tolist()}")
