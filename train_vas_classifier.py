#!/usr/bin/env python3
"""
train_vas_classifier.py
=======================
Train and evaluate a binary VAS-phase classifier on top of SAM2 ViT features.

Input: .npz files produced by extract_vas_features.py (one per case).
Protocol: leave-one-case-out (LOCO) cross-validation — train on N-1 cases, test on 1.
          This gives a realistic estimate of how well the model generalises to a new patient.

Models tried (in order of complexity):
  1. Logistic regression   — linear probe, tells you if the feature space is linearly separable
  2. MLP (1 hidden layer)  — non-linear probe, ~10 mins training

Metrics: AUC-ROC (primary), F1, accuracy, confusion matrix.
Output: results printed per fold + aggregate, confusion matrix heatmap saved as PNG.

Usage
-----
  python3 train_vas_classifier.py
  python3 train_vas_classifier.py --features_dir custom_features/ --model mlp
  python3 train_vas_classifier.py --model both
"""
...
FEATURES_DIR_DEFAULT = os.path.join(SAM3_DIR, "vas_features")


# ── Data loading ───────────────────────────────────────────────────────────────

def load_all_cases(features_dir):
    """Returns list of (case_id, features [N, D], labels [N]) sorted by case_id."""
    cases = []
...
def loco_cv(cases, model_type, C=1.0, hidden=256):
    ...
    Returns list of per-fold dicts with keys: case_id, auc, f1, acc, y_true, y_prob.
    """
...
def confusion_matrix_plot(results, model_type, out_png):
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm,
                                  display_labels=["Other", "VAS dissection"])
...
def roc_plot(results, model_type, out_png):
    ...
    ax.set_title(f"ROC — VAS dissection classifier ({model_type}, LOCO-CV)")
...
def main():
    ...
    if not cases:
        print("No .npz files found. Run extract_vas_features.py first.")
        return
