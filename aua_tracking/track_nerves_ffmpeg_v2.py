#!/usr/bin/env python3
"""
track_nerves_ffmpeg_v2.py
=========================

Same as track_nerves_ffmpeg_old.py with two fixes for videos where
actual fps != annotation fps (e.g. 59.94 fps video annotated at 30 fps):

FIX 1 — Sub-chunk sizing in actual frames, not annotation frames.
  The old script split by MAX_CACHED_FRAMES annotation frames.  For a 2x
  fps ratio this loaded ~2x as many actual frames as expected, filling
  the GPU and causing OOM.  This version uses MAX_ACTUAL_FRAMES (actual
  video frames) and derives the annotation-frame chunk size as:
      ann_chunk_size = round(MAX_ACTUAL_FRAMES * fps_ann / fps_actual)
  so every sub-chunk encodes at most MAX_ACTUAL_FRAMES actual frames
  regardless of the fps ratio.

FIX 2 — global_idx scaled by fps ratio.
  The old script saved mask k at annotation frame sub_start + k.  With
  fps_actual > fps_ann the loader returns more actual frames than
  annotation frames in the sub-chunk, so later sub-chunks overwrote
  earlier ones with masks from different video times, causing visible
  drift at each sub-chunk boundary.  The fix:
      global_idx = sub_start + round(local_idx * frame_step * fps_ann / fps_actual)
  maps actual-frame indices back to annotation-frame indices correctly.
  When fps_actual == fps_ann the ratio is 1.0 and behaviour is identical
  to the old script.

Tune MAX_ACTUAL_FRAMES after running benchmark_sam2_frames.py.
500 is a conservative default for SAM2-large on H100 80 GB.

Usage (identical to old script):

python3 aua_tracking/track_nerves_ffmpeg_v2.py \
...
def _ffmpeg_read_bgr(video_path: str, time_s: float,
                     ...
                     sbs_eye_x: int = 0, sbs_eye_w: Optional[int] = None) -> Optional[np.ndarray]:
    ...
    expected = eye_w * video_h * 3
    if len(result.stdout) < expected:
        return None
    return np.frombuffer(result.stdout, dtype=np.uint8).reshape(video_h, eye_w, 3).copy()


def _ffmpeg_frame_gen(video_path: str, start_time_s: float, duration_s: float,
                      video_w: int, video_h: int):
    """Yield full-width BGR frames sequentially via ffmpeg pipe."""
...
class FFmpegChunkLoader:
    ...
    def __getitem__(self, local_idx: int):
        ...
        frame = self._frames[local_idx]  # (image_size, image_size, 3) uint8 RGB
...
class LazyMaskCache:
    ...
    def __init__(self, mask_dir: str, video_h: int, eye_w: int):
    ...
    def _load(self, fidx: int) -> np.ndarray:
        if fidx not in self._cache:
            ...
            if m.shape[:2] != (self._video_h, self._eye_w):
                m = cv2.resize(m, (self._eye_w, self._video_h),
    ...
    def get_mask_at(self, fidx: int) -> Optional[np.ndarray]:
        if not self.kf_idxs:
            return None
...
def main():
    ...
    # FIX 1: derive annotation-frame chunk size from actual-frame budget.
    # fps_ratio > 1 when the video is higher fps than the annotation tool used.
    ...
    for track in prepared_tracks:
        ...
        for mi, m in enumerate(mask_entries):
            ...
            mask_eye  = crop_mask_to_eye(
                mask_full=mask_full, video_h=video_h, video_w=video_w,
                eye_x=eye_x, eye_w=eye_w, sbs=sbs,
            )
            if not mask_eye.any():
        ...
        for si, ((seed_frame, seed_mask), seg_start, seg_end) in enumerate(
            zip(key_masks_eye, seg_starts, seg_ends), start=1,
        ):
            ...
            # FIX 1: sub-chunk boundaries in annotation frames, sized so each
            # chunk loads at most MAX_ACTUAL_FRAMES actual video frames.
            ...
            for sub_i, sub_start in enumerate(sub_starts):
                ...
                with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                    ...
                    for local_idx, _, mask_logits in predictor.propagate_in_video(state):
                        ...
                        # FIX 2: map actual-frame index back to annotation-frame index.
                        # When fps_ratio == 1.0 this is identical to sub_start + local_idx.
                    ...
                    try:
                        ...
                        pass
                    # reset_state clears SAM2 tracking outputs but NOT cached_features.
                    # Explicitly clear the GPU tensors before releasing the state so
                    # memory is freed before the next sub-chunk or next track allocates.
    ...
    for track in prepared_tracks:
        ...
        mask_cache = LazyMaskCache(tmd, video_h, eye_w)
        color_bgr = (
    ...
    # The renderer outputs at fps (actual fps); each consumed actual frame maps
    # to a fractional ann-frame coordinate, which is used for mask lookup and
    # track-active range checks. Fixes the alignment bug where fidx was treated
    # as ann index but each iteration consumed one actual frame, so masks ran at
    # fps_actual/fps_ann the correct speed.
    ...
    for out_idx in range(n_render_actual):
        ...
        for track in prepared_tracks:
            ...
            frame[:, eye_x:eye_x + eye_w] = (eye_slice * (1.0 - alpha) + color_layer * alpha).astype(np.uint8)
            if sbs and other_x is not None:
                other_slice = frame[:, other_x:other_x + eye_w].astype(np.float32)
                frame[:, other_x:other_x + eye_w] = (other_slice * (1.0 - alpha) + color_layer * alpha).astype(np.uint8)
