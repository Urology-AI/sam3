#!/usr/bin/env python3
"""
compare_sam2base_vs_finetuned.py
=================================
Side-by-side comparison on the chahat clip:

  Left  → SAM2 base tracker   (fine-tuned detector box → add_new_points_or_box → propagate)
  Right → SAM2 + fine-tuned decoder
           (fine-tuned detector box → noise-robust SAM2 decoder → add_new_mask → propagate)

The right-panel masks are pre-computed by infer_prostate.py and loaded directly.
The left-panel masks are regenerated here via chunked reinit at the same interval.

Usage:
  cd /sc/arion/projects/video_rarp/neel_projects/segmentation_prostate/sam3
  python3 compare_sam2base_vs_finetuned.py
"""
...
# Pre-computed fine-tuned decoder masks (from infer_prostate.py)
...
SAM2_BASE_MASKS  = os.path.join(OUTPUT_DIR, "_sam2_base_masks")  # kept after run

TEXT_QUERY        = "prostate gland"
DETECTOR_IMG_SIZE = 1008
REINIT_INTERVAL   = 150   # frames (~2.5 s at 60 fps)
MASK_ALPHA        = 0.15
...
# ── Streaming chunk loader (identical to compare_sam2track_vs_sam3.py) ─────────
class VideoChunkLoader:
    _MEAN = torch.tensor([0.485, 0.456, 0.406])[:, None, None]
    _STD  = torch.tensor([0.229, 0.224, 0.225])[:, None, None]

    def __init__(self, video_path, image_size, start_frame=0, n_frames=None):
...
def main():
    ...
    os.makedirs(SAM2_BASE_MASKS, exist_ok=True)
    ...
    # Verify fine-tuned masks exist
    ft_count = len([f for f in os.listdir(FINETUNED_MASKS_DIR) if f.endswith(".npy")])
    print(f"Fine-tuned decoder masks found: {ft_count}  ({FINETUNED_MASKS_DIR})")
    ...
    print(f"Reinit every {REINIT_INTERVAL} frames ({REINIT_INTERVAL/fps:.1f} s)"
          f"  →  {len(chunk_starts)} chunks")

    # ── [1] Pre-detect boxes at all reinit boundaries ─────────────────────────
    print(f"\n[1/4] Detecting prostate box at {len(chunk_starts)} reinit frames...")
    t0 = time.time()
    ...
    # ── [2] SAM2 base chunked propagation → save masks ─────────────────────────
    print(f"\n[2/4] Loading SAM2 large video predictor (base)...")
    ...
    print(f"\n[3/4] SAM2 base chunked propagation ({len(chunk_starts)} chunks)...")
    t0         = time.time()
    mask_count = 0
    ...
    for ci, cs in enumerate(chunk_starts):
        ...
        with torch.inference_mode():
            ...
            for local_idx, _, mask_logits in predictor.propagate_in_video(state):
                ...
                if mask.any():
                    np.save(os.path.join(SAM2_BASE_MASKS, f"{global_idx:06d}.npy"), mask)
                    chunk_masks += 1
                    mask_count  += 1
            try:
        ...
        print(f"  chunk {ci+1:2d}/{len(chunk_starts)}  "
              f"frames {cs}-{cs+chunk_size-1}  masks={chunk_masks}  ({elapsed:.0f}s)")

    del predictor
    torch.cuda.empty_cache()
    print(f"  Propagation done: {mask_count} SAM2 base masks total")

    # ── [3] Render side-by-side ────────────────────────────────────────────────
    print(f"\n[4/4] Rendering side-by-side video...")
    out_raw  = os.path.join(OUTPUT_DIR, "_raw_cmp_finetuned.mp4")
    out_path = os.path.join(OUTPUT_DIR, "comparison_sam2base_vs_finetuned.mp4")
    ...
    for fidx in range(n_frames):
        ...
        # ── Left: SAM2 base tracker ──────────────────────────────────────────
        left = frame.copy()
        npy  = os.path.join(SAM2_BASE_MASKS, f"{fidx:06d}.npy")
        ...
        for cs, (box_abs, conf) in reinit_boxes.items():
            if cs <= fidx < cs + box_dur:
                ...
                break
        put_label(left, f"SAM2 Base Tracker  (box prompt only, reinit {REINIT_INTERVAL/fps:.1f}s)")

        # ── Right: SAM2 + fine-tuned decoder ────────────────────────────────
        right = frame.copy()
        npy   = os.path.join(FINETUNED_MASKS_DIR, f"{fidx:06d}.npy")
        if os.path.exists(npy):
            m = np.load(npy)
            if m.shape[:2] != (h_vid, w_vid):
                m = cv2.resize(m, (w_vid, h_vid), interpolation=cv2.INTER_NEAREST)
            right = apply_mask(right, m, MASK_COLOR_BGR, MASK_ALPHA)
        put_label(right, f"SAM2 + Fine-tuned Decoder  (reinit {REINIT_INTERVAL/fps:.1f}s)")
    ...
    print(f"  Base masks:    {mask_count}/{n_frames}  →  {SAM2_BASE_MASKS}/")
    print(f"  Finetuned:     {ft_count} masks  →  {FINETUNED_MASKS_DIR}/")
